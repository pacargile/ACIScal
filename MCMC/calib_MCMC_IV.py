...
class ACIS(object):
    ...
    def logprior(self,t):
        ...
        # free parameters
        Teff,Dist,Rad,NH,pararf1,pararf2,pararf3,pararf4 = t
        ...
        PTeff = -0.5 * (((Teff-self.Teff)**2.0)/(self.eTeff**2.0)) + np.log(1.0/np.sqrt(2*np.pi*(self.eTeff**2.0)))
        PRad  = -0.5 * (((Rad-self.Rad)**2.0)/(self.eRad**2.0)) + np.log(1.0/np.sqrt(2*np.pi*(self.eRad**2.0)))
        PDist = -0.5 * (((Dist-self.Dist)**2.0)/(self.eDist**2.0)) + np.log(1.0/np.sqrt(2*np.pi*(self.eDist**2.0)))
        PNH   = -0.5 * (((NH-self.NH)**2.0)/(self.eNH**2.0)) + np.log(1.0/np.sqrt(2*np.pi*(self.eNH**2.0)))

        return PTeff+PRad+PDist+PNH
    ...
    def run_MCMC(self):
        ...
        # Tefff, Dist, Rad, N_H, arf1, arf2, arf3, arf4
        p0mean = [143431.928537, 0.410281810919, 0.0333451376657, 0.05,  self.arf1, self.arf2, self.arf3, self.arf4]
        p0std  = [2000.0,        0.01,           0.01,            0.001, 5.0,       5.0,      5.0,        5.0]
        ...
        with open(fn, "w") as f:
            f.write('WN Teff Dist Rad NH arf1 arf2 arf3 arf4 lnprob\n')
