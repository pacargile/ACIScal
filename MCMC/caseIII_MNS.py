...
class ACIS(object):
    ...
    def prior_trans(self,t):
        Teff,Dist,Rad,NH,arfsc1,arfsc2,arfsc3,arfsc4 = t
        ...
        outarr = np.array([
            self.Teff + self.eTeff*ndtri(Teff),
            self.Dist + self.eDist*ndtri(Dist),
            self.Rad + self.eRad*ndtri(Rad),
            self.NH + self.eNH*ndtri(NH),
            arfscran * arfsc1 + arfscmin,
            arfscran * arfsc2 + arfscmin,
            arfscran * arfsc3 + arfscmin,
            arfscran * arfsc4 + arfscmin,
            ])
        return outarr
    ...
    def loglhood(self,t):
        ...
        for ii,ee_i in enumerate(self.e):
            if (ee_i >= 0.175) and (ee_i < 0.23):
                ...
                arf[ii] = self.arf2*arfsc2
            elif (ee_i >= 0.26) and (ee_i < 0.287):
                arf[ii] = self.arf3*arfsc3
            elif (ee_i >= 0.287):
                arf[ii] = self.arf4*arfsc4
    ...
    def run_nestle(self,outfile='TEST.dat'):
        ...
        self.outfile.write('ITER Teff Dist Rad NH arfsc1 arfsc2 arfsc3 arfsc4 log(z) \n')
        # Start sampler
        print('Start Nestle')
...
if __name__ == '__main__':
    ...
    c = ACIS()
    # outfile names
    outfile = 'ACIScal_I_MNS_CASEIII.out'
    outh5file = 'ACIScal_I_MNS_CASEIII.h5'
    # run the MCMC
