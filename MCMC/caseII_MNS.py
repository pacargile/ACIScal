    ...
    def loglhood(self,t):
        ...
        for ii,ee_i in enumerate(self.e):
            if (ee_i >= 0.175) and (ee_i < 0.23):
                arf[ii] = self.arf1*arfsc
            elif (ee_i >= 0.23) and (ee_i < 0.26):
                arf[ii] = self.arf2*arfsc
            elif (ee_i >= 0.26) and (ee_i < 0.287):
                arf[ii] = self.arf3*arfsc
            elif (ee_i >= 0.287):
                arf[ii] = self.arf4*arfsc
...
if __name__ == '__main__':
    ...
    c = ACIS()
    # outfile names
    outfile = 'ACIScal_I_MNS_CASEII.out'
    outh5file = 'ACIScal_I_MNS_CASEII.h5'
    # run the MCMC
